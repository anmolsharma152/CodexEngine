...
def rewrite_query_with_context(
    api_key, provider, model_name, chat_history, current_query
):
    """
...
def generate_answer(
    api_key, provider, model_name, documents, metadatas, chat_history, current_query
):
    try:
        ...
        response = completion(
            model=f"{provider.lower()}/{model_name}", messages=messages
        )
