...
def generate_answer(state: AgentState) -> dict:
    ...
    is_academic = any(
        ...
        for term in ["framework", "define", "concept", "theory", "ssl", "error rate"]
    )

    role = "Technical Analyst" if is_academic else "Narrative Analyst"
    focus = (
        "technical accuracy and definitions"
        if is_academic
        else "character motivation and plot triggers"
    )

    # Check if we actually retrieved anything useful
    if not state.context or len(state.context) == 0:
        prompt = f"""
        You are CodexEngine, a helpful AI assistant.
        The user said: {state.query}
        Simply respond naturally and politely. Do not cite sources.
        """
    else:
        # Your existing logic
        role = "Technical Analyst" if is_academic else "Narrative Analyst"
